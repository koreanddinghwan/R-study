...
# 칼럼값에 대해 결측치를 Unknown으로 바꾸하는 사용자 정의 함수
# 사용자 정의함수 배웠으니, 사용하겠습니다.
...
#결측치 처리 사용자 정의 함수
#boxplot->stats 사용
fnc_extreme_to_na <- function(variable){
...
df_ms$sleep_cycle <- fnc_extreme_to_na(df_ms$sleep_cycle)
table(is.na(df_ms$sleep_cycle))

df_ms$sleep_rem <- fnc_extreme_to_na(df_ms$sleep_rem)
table(is.na(df_ms$sleep_rem))

#극단치를 벗어난 값들이 결측치화되었다.
...
#따로 데이터프레임을 정의해서 사용할수도 있지만, 
#메모리 상에 굳이 남아있을 필요가 없을 것 같아 확인만하고, ggplot에 바로 넣어주겠습니다.
#df_ms %>%
#		group_by(vore) %>%
...
#table(is.na(df_ms$order))
#결측치 없으므로, inner_join 써도되나, order를 key값으로 join
...
df_ms <- df_ms %>%
	mutate(nonrem_ratio =
			ifelse(is.na(df_ms$sleep_nonrem),
				NA,
